import { verifyFilm } from '../library/tmdb.js';
...
Rules:
- Write plain text only in every field: no links, citations, footnotes or markdown. Sources are collected separately.
- premise: 2-4 sentences (about 80 words at most) describing only the setup and stakes. Nothing that happens after the first act, and no deaths or endings. Never reveal the film's central secret or twist, even when it is revealed early or shown in trailers: say what question the story raises, not what the answer is.
...
/** Pull the JSON payload and the real citation sources out of a Responses API result. */
export function parseResponse(response) {
  ...
  return { data, sources: collectSources(textParts) };
}

/** Sources come from the API's own url_citation annotations, never from model-written fields. */
function collectSources(textParts) {
  const sources = new Map();
  for (const part of textParts) {
    for (const annotation of part.annotations ?? []) {
      if (annotation.type !== 'url_citation') continue;
      const url = cleanUrl(annotation.url);
      if (url && !sources.has(url)) sources.set(url, { title: annotation.title, url });
    }
  }
 ...
 * canonical title and year it returns. Drops the film itself, repeats, and anything already
 * shown in `alsoShown` (the other list), then caps the list.
 * Rejects if verification fails, so an outage is not mistaken for "every film is invented".
 */
export async function verifyRecommendations(recs, { verify, selfTmdbId, max, alsoShown = [] }) {
  const verified = await Promise.all(recs.map((rec) => verify(rec.title, rec.year)));
  const seen = new Set([selfTmdbId]);
...
async function generate(facts, { apiKey, model, timeoutMs, now }) {
  ...
  const { data, sources } = parseResponse(response);
